...
__device__ void compute_transmat(
	...
	//normal = {L[0][2], L[1][2], L[2][2]};
	//normal = {L[2].x, L[2].y, L[2].z};
}
	...
	bool prefiltered)
{
	...
	///if (!front_facing(normal, p_view, prefiltered)) return; // cull backfacing points
	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += brdf[collected_id[j] * CHANNELS + ch] * w;
	...
	if (inside)
	{
		...
		for (int ch=0; ch<S; ch++) out_others[pix_id + (BASIS_OFFSET+ch) * H * W] = F[ch];
