...
__forceinline__ __device__ bool front_facing(
	...
	bool prefiltered)
{
	float dot = p_view.x * normal.x + p_view.y * normal.y + p_view.z * normal.z;
	// float z = normal.z;
	bool cond = (dot > -0.01);
...
inline __device__ glm::vec4 quat_to_rotmat_vjp(const glm::vec4 quat, const glm::mat3 v_R) {
	float s = rsqrtf(
		quat.w * quat.w + quat.x * quat.x + quat.y * quat.y + quat.z * quat.z
	);
	float w = quat.x * s;
...
inline __device__ glm::mat3 scale_to_mat(const glm::vec2 scale, const float glob_scale) {
