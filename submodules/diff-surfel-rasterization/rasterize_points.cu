	...
	const bool debug)
{
  ...
  torch::Tensor out_others = torch::full({3+3+1+S, H, W}, 0.0, float_opts);
	...
	const bool debug) 
{
  ...
  if(P != 0)
  {  
	  CudaRasterizer::Rasterizer::backward(P,S, R,
	  background.contiguous().data<float>(),
	  W, H, 
	  means3D.contiguous().data<float>(),
	  colors.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dscales, dL_drotations, dL_dfeatures);
}
